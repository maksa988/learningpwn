...
#define     SERVER_MODE		"Advance RP v2.20"//название мода
...
public OnPlayerText(playerid, text[])
{
	...
	if(!strcmp(text, ")", true))
	{
	    ...
	    SetPlayerChatBubble(playerid, "улыбается", 0xe782FFFF, 15, 7000);
	    return 0;
	}
	if(!strcmp(text, "))", true))
	{
	    ...
	    ProxDetector(30.0, playerid, string, 0xe782FFFF, 0xe782FFFF, 0xe782FFFF, 0xe782FFFF, 0xe782FFFF);
	    SetPlayerChatBubble(playerid, "смеётся", 0xe782FFFF, 15, 7000);
	    return 0;
	}
	if(!strcmp(text, "(", true))
	{
	    ...
	    ProxDetector(30.0, playerid, string, 0xe782FFFF, 0xe782FFFF, 0xe782FFFF, 0xe782FFFF, 0xe782FFFF);
	    SetPlayerChatBubble(playerid, "расстроился", 0xe782FFFF, 15, 7000);
	    return 0;
	}
	if(!strcmp(text, "((", true))
	{
	    ...
	    SetPlayerChatBubble(playerid, "сильно расстроился", 0xe782FFFF, 15, 7000);
	    return 0;
	}
	if(!strcmp(text, "=0", true))
	{
	    ...
	    ProxDetector(30.0, playerid, string, 0xe782FFFF, 0xe782FFFF, 0xe782FFFF, 0xe782FFFF, 0xe782FFFF);
	    SetPlayerChatBubble(playerid, "удивился", 0xe782FFFF, 15, 7000);
	    return 0;
	}
	...
	if(chat < 93)
	{
		...
		if(GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
		{
		    ...
		    ApplyAnimation(playerid, "PED", "IDLE_chat", 4.1, 0, 1, 1, 1, 1);
		 	...
		 	SetPlayerChatBubble(playerid, text, 0xFFFFFFFF, 15, 7000);
		}
	}
	return 0;
}
		    ...
		    if(response)//левая кнопка
			{
                ...
                switch(listitem)
			    {
			        case 0..6:
			        {
						...
						static const fmt_str[] = "~g~+%d$";
						new string[sizeof(fmt_str)+(-2+5)];
						format(string, sizeof(string), fmt_str, money);
...
CMD:n(playerid, params[])
{
	...
	SetPlayerChatBubble(playerid, string, 0xCCCC99FF, 15, 7000);
	return 1;
}
CMD:me(playerid, params[])
{
	...
	SetPlayerChatBubble(playerid, string, 0xDD90FFFF, 15, 7000);
	return 1;
}
...
CMD:s(playerid, params[])
{
	...
	if(GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
   		...
   		SetPlayerChatBubble(playerid, params[0], COLOR_WHITE, 15, 7000);
    }
	return 1;
}
CMD:exit(playerid)
...
CMD:w(playerid,params[])
{
	...
	ProxDetector(2.0, playerid, string, 0xACCE90FF, 0xACCE90FF, 0xACCE90FF, 0xACCE90FF, 0xACCE90FF);
	SetPlayerChatBubble(playerid, string, 0xACCE90FF, 15, 2000);
	return 1;
}
//================================================================================================
...
CMD:kick(playerid, params[])
{
	...
	new string[128];
	if(sscanf(params, "dS()[38]", params[0], params[1])) return SCM(playerid, COLOR_LIGHTGREY, "Используй /kick [id] [Причина(не обязательно)]");
	if(GetPVarInt(params[0], "logged") != 1) return SCM(playerid, COLOR_LIGHTGREY, "Игрока с таким айди нет");
...
{
    if(player_info[playerid][ADMIN] < 2) return SCM(playerid, COLOR_LIGHTGREY, "Эта команда недоступна на вашем уровне администратора");
	if(sscanf(params, "d", params[0])) return SCM(playerid, COLOR_LIGHTGREY, "Используй /skick [id]");
	new string[128];
	if(GetPVarInt(params[0], "logged") != 1) return SCM(playerid, COLOR_LIGHTGREY, "Игрока с таким айди нет");
...
{
    if(player_info[playerid][ADMIN] < 4) return SCM(playerid, COLOR_LIGHTGREY, "Эта команда недоступна на вашем уровне администратора");
    if(sscanf(params, "d", params[0])) return SCM(playerid, COLOR_LIGHTGREY, "Используй /gethere [id]");
    if(GetPVarInt(params[0], "logged") != 1) return SCM(playerid, COLOR_LIGHTGREY, "Игрока с таким айди нет");
...
{
	if(player_info[playerid][ADMIN] < 1) return 1;
	if(sscanf(params, "ds[49]", params[0], params[1])) return SCM(playerid, COLOR_LIGHTGREY, "Используй /ans [id] [сообщение]");
	if(GetPVarInt(params[0], "logged") != 1) return SCM(playerid, COLOR_LIGHTGREY, "Игрока с таким айди нет");
	new string[128];
...
{
	if(player_info[playerid][ADMIN] < 4) return SCM(playerid, COLOR_LIGHTGREY, "Эта команда недоступна на вашем уровне администратора");
	if(sscanf(params, "dd", params[0], params[1])) return SCM(playerid, COLOR_LIGHTGREY, "Используй /skin [id] [id внешности]");
	if(GetPVarInt(params[0], "logged") != 1) return SCM(playerid, COLOR_LIGHTGREY, "Игрока с таким айди нет");
...
{
    if(player_info[playerid][ADMIN] < 2) return SCM(playerid, COLOR_LIGHTGREY, "Эта команда недоступна на вашем уровне администратора");
    if(sscanf(params, "ddS()[32]", params[0], params[1], params[2])) return SCM(playerid, COLOR_LIGHTGREY, "Используй /mute [id] [время] [причина]");
    if(GetPVarInt(params[0], "logged") != 1) return SCM(playerid, COLOR_LIGHTGREY, "Игрока с таким айди нет");
...
{
    if(player_info[playerid][ADMIN] < 2) return SCM(playerid, COLOR_LIGHTGREY, "Эта команда недоступна на вашем уровне администратора");
    if(sscanf(params, "d", params[0])) return SCM(playerid, COLOR_LIGHTGREY, "Используй /unmute [id]");
    if(GetPVarInt(params[0], "logged") != 1) return SCM(playerid, COLOR_LIGHTGREY, "Игрока с таким айди нет");
...
CMD:warn(playerid,params[])
{
    ...
    if(sscanf(params, "dS()[34]", params[0], params[1])) return SCM(playerid, COLOR_LIGHTGREY, "Используй /warn [id] [причина]");
...
{
    if(player_info[playerid][ADMIN] < 1) return 1;
    if(sscanf(params, "d", params[0])) return SCM(playerid, COLOR_LIGHTGREY, "Используй /sp [id]");
    if(GetPVarInt(params[0], "logged") != 1) return SCM(playerid, COLOR_LIGHTGREY, "Игрока с таким айди нет");
...
{
    if(player_info[playerid][ADMIN] < 3) return SCM(playerid, COLOR_LIGHTGREY, "Эта команда недоступна на вашем уровне администратора");
    if(sscanf(params, "d", params[0])) return SCM(playerid, COLOR_LIGHTGREY, "Используй /goto [id]");
    if(GetPVarInt(params[0], "logged") != 1) return SCM(playerid, COLOR_LIGHTGREY, "Игрока с таким айди нет");
    new Float:tpx, Float:tpy, Float:tpz;
...
{
    if(player_info[playerid][ADMIN] < 4) return SCM(playerid, COLOR_LIGHTGREY, "Эта команда недоступна на вашем уровне администратора");
    if(sscanf(params, "dd", params[0], params[1])) return SCM(playerid, COLOR_LIGHTGREY, "Используй /hp [id] [уровень hp]");
    if(GetPVarInt(params[0], "logged") != 1) return SCM(playerid, COLOR_LIGHTGREY, "Игрока с таким айди нет");
...
{
    if(player_info[playerid][ADMIN] < 3) return SCM(playerid, COLOR_LIGHTGREY, "Эта команда недоступна на вашем уровне администратора");
    if(sscanf(params, "ddS()[26]", params[0], params[1], params[2])) return SCM(playerid, COLOR_LIGHTGREY, "Используй /ban [id] [срок] [причина]");
    if(GetPVarInt(params[0], "logged") != 1) return SCM(playerid, COLOR_LIGHTGREY, "Игрока с таким айди нет");
...
CMD:sskin(playerid, params[])
{
    ...
    if(sscanf(params, "dd", params[0], params[1])) return SCM(playerid, COLOR_LIGHTGREY, "Используй /sskin [id] [id скина]");
    if(GetPVarInt(params[0], "logged") != 1) return SCM(playerid, COLOR_LIGHTGREY, "Игрока с таким айди нет");
...
CMD:offwarn(playerid, params[])
{
    ...
    if(sscanf(params, "s[24]S()", warnnick, warnreason)) return SCM(playerid, COLOR_LIGHTGREY, "Используй /offwarn [ник] [причина]");
    foreach(new i:Player)
	{
...
CMD:offban(playerid, params[])
{
    ...
    if(sscanf(params, "s[24]dS()", bannick, bantime, banreason)) return SCM(playerid, COLOR_LIGHTGREY, "Используй /offban [ник] [срок] [причина]");
