...
new bustimer[MAX_PLAYERS]; //таймер 15 секунд на возвращение в авто
new nextcheckbustimer[MAX_PLAYERS]; // таймер на 10 секунд автобусной остановки
new taxicounter_timer[MAX_PLAYERS]; //таймер 30 секунд на тариф
new taxiveh[MAX_PLAYERS]; //сохранённая машина такси
new busveh[MAX_PLAYERS]; //сохранённый автобус
...
//---------------------Маршруты автобусов-------------------
new Float:buscheckOne[46][3] = //координаты чекпоинтов Маршрута (Городской Лос-Сантос №1)
{
	{1171.3560,-1759.5508,13.4922}, // STOP
	{1171.5439,-1776.3657,13.4922},
	{1173.8173,-1843.8954,13.4980},
	{1241.2742,-1855.0886,13.4770},
	{1306.0314,-1855.8632,13.4834},
	{1315.1796,-1741.1396,13.4852},
	{1451.9539,-1735.2166,13.4752}, // STOP
	{1468.4813,-1735.2418,13.4841},
	{1808.0092,-1734.7067,13.4866},
	{1818.0129,-1867.4036,13.5062}, // STOP
    {1818.4779,-1882.4570,13.4940},
    {1832.7573,-1935.1310,13.4858},
    {1958.8157,-1934.7223,13.4949},
    {1958.9398,-2141.0256,13.4889}, // STOP
    {1960.3464,-2161.4060,13.4834},
    {2139.2461,-2216.9866,13.4952},
    {2191.8369,-2174.3186,13.4867},
    {2245.3203,-2216.7280,13.4107}, // STOP
    {2260.2620,-2230.5332,13.4575},
    {2298.4041,-2266.3857,13.4697},
    {2469.5994,-2172.3418,13.6161},
    {2758.6675,-2164.5840,11.0332},
    {2848.6614,-2049.6030,11.0297},
    {2861.2556,-1986.3341,11.0384}, // STOP
    {2860.8655,-1972.3917,11.0366},
    {2832.7012,-1888.6583,11.0272},
    {2686.5647,-1876.5034,10.9919},
    {2645.8501,-1678.8838,10.8832}, // STOP
    {2645.9941,-1664.2544,10.8358},
    {2645.0017,-1065.9069,69.5502},
    {2612.0107,-1044.9880,69.5110}, // STOp
    {2594.3967,-1045.0535,69.5148},
    {2559.7029,-1066.2637,69.1805},
    {2323.0146,-1141.2725,27.0661},
    {2236.8862,-1133.1836,25.7422}, // STOP
    {2221.6812,-1129.6388,25.7296},
    {1879.2019,-1044.5654,23.7844},
    {1857.6768,-1176.9501,23.7914},
    {1656.8127,-1157.7115,23.8686}, // STOP
    {1642.2625,-1158.1434,24.0049},
    {1354.0316,-1141.4788,23.7626},
    {1339.4939,-1306.3466,13.5624}, // STOP
    {1339.7743,-1320.8105,13.4913},
    {1309.9259,-1511.2142,13.4905},
...
public OnVehicleSpawn(vehicleid)
{
	...
	if(vehicleid <= buscars[0] && vehicleid >= buscars[MAX_BUS_CARS])
	{
    	Delete3DTextLabel(bustext[vehicleid]);
	}
	return 1;
}
...
public OnPlayerExitVehicle(playerid, vehicleid)
{
	...
	if(GetPlayerVehicleID(playerid) >= buscars[0] && GetPlayerVehicleID(playerid) <= buscars[MAX_BUS_CARS])
	{
	    if(GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
	 	{
		    SCM(playerid, COLOR_ORANGE, "У Вас есть 15 секунд чтобы вернуться в автобус");
		    bustimer[playerid] = SetTimerEx("busworkend", 15000, false, "i", playerid);
		}
	}
	return 1;
}
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	if(newstate == PLAYER_STATE_DRIVER)
	{
        ...
        if(GetPlayerVehicleID(playerid) >= buscars[0] && GetPlayerVehicleID(playerid) <= buscars[MAX_BUS_CARS])
	    {
	        if(player_info[playerid][WORK] == 1)
	        {
				if(GetPVarInt(playerid, "bus_work") == 1) {
					KillTimer(bustimer[playerid]);
				} else {
					SPD(playerid, DIALOG_BUS, DIALOG_STYLE_MSGBOX, "{ffcd00}Автобусный парк", "{ffffff}Что бы взять машину, необходимо заплатить городскому автопарку 180$\nВы действительно хотите взять автобус в аренду?", "Да", "Нет");
				}
			}
	        else
	        {
	...
	if(newstate == PLAYER_STATE_PASSENGER) {
        if(GetPlayerVehicleID(playerid) >= buscars[0] && GetPlayerVehicleID(playerid) <= buscars[MAX_BUS_CARS])
	    {
			if(GetPVarInt(playerid, "bus_work") == 1) return RemovePlayerFromVehicle(playerid);
            if(GetPVarInt(jobdriver[GetPlayerVehicleID(playerid)], "bus_route") == 0) return 1;
            
            if(player_info[playerid][MONEY] < GetPVarInt(jobdriver[GetPlayerVehicleID(playerid)], "bus_fare")) {
			    RemovePlayerFromVehicle(playerid);
				SCM(playerid, COLOR_LIGHTGREY, "У вас недостаточного денег для проезда.");
				return 1;
			}
			player_info[playerid][MONEY]-= GetPVarInt(jobdriver[GetPlayerVehicleID(playerid)], "bus_fare");
			add_to_salary(jobdriver[GetPlayerVehicleID(playerid)], GetPVarInt(jobdriver[GetPlayerVehicleID(playerid)], "bus_fare"));

			SetPVarInt(jobdriver[GetPlayerVehicleID(playerid)], "bus_salary", GetPVarInt(jobdriver[GetPlayerVehicleID(playerid)], "bus_salary") + GetPVarInt(jobdriver[GetPlayerVehicleID(playerid)], "bus_fare"));

			new stringOne[8];
			format(stringOne, sizeof(stringOne), "~g~+%d$", GetPVarInt(jobdriver[GetPlayerVehicleID(playerid)], "bus_fare"));
			GameTextForPlayer(jobdriver[GetPlayerVehicleID(playerid)], stringOne, 3000, 1);

			new stringTwo[8];
			format(stringTwo, sizeof(stringTwo), "~r~-%d$", GetPVarInt(jobdriver[GetPlayerVehicleID(playerid)], "bus_fare"));
			...
			SetPVarInt(jobdriver[GetPlayerVehicleID(playerid)], "bus_passengers", GetPVarInt(jobdriver[GetPlayerVehicleID(playerid)], "bus_passengers") + 1);
   		}
		...
		if(GetPlayerVehicleID(playerid) >= taxicars[0] && GetPlayerVehicleID(playerid) <= taxicars[MAX_TAXI_CARS])
	    {
			...
			switch(GetPVarInt(jobdriver[GetPlayerVehicleID(playerid)], "taxi_type")) { //По типу определяем текст вывода и тариф
			    ...
			    case 2:
   				{
   				    new stringOne[58];
					format(stringOne, sizeof(stringOne), "%s сел в ваше такси. Счетчик включен", player_info[playerid][NAME]);
					SCM(jobdriver[GetPlayerVehicleID(playerid)], COLOR_LIGHTBLUE, stringOne);
					
					new stringTwo[86];
					format(stringTwo, sizeof(stringTwo), "Вы сели в такси %s. Тариф %d$ (оплата каждые 30 секунд поездки)", player_info[jobdriver[GetPlayerVehicleID(playerid)]][NAME], GetPVarInt(jobdriver[GetPlayerVehicleID(playerid)], "taxi_fare"));
					SCM(playerid, COLOR_LIGHTBLUE, stringTwo);
        ...
        if(GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
	 	{
			if(GetPlayerVehicleID(playerid) >= buscars[0] && GetPlayerVehicleID(playerid) <= buscars[MAX_BUS_CARS])
			{
				if(GetPVarInt(playerid, "bus_route") == 1) { //Городской Лос-Сантос №1
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	...
	if(GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
	{
        if(newkeys & KEY_SUBMISSION) // 2 (субмиссия)
        {
			...
			if(GetPlayerVehicleID(playerid) >= buscars[0] && GetPlayerVehicleID(playerid) <= buscars[MAX_BUS_CARS] && GetPVarInt(playerid, "bus_work") == 1) //Автобус
            {
				if(GetPVarInt(playerid, "bus_route") != 0) {
					SPD(playerid, DIALOG_BUS + 3, DIALOG_STYLE_MSGBOX, "{FFEF0D}Автобусный парк", "{ffffff}Вы уверены что хотите завершить рабочий день?", "Да", "Нет");
				} else {
				    busveh[playerid] = GetPlayerVehicleID(playerid);
					SPD(playerid, DIALOG_BUS + 1, DIALOG_STYLE_INPUT, "{ffcd00}Стоимость проезда", "{ffffff}Сколько будет стоить проезд?\nУкажите цену (0$ - 100$)", "Далее", "Отмена");
				}
			}
        }
 	...
 	switch(dialogid)
 	{
		...
		case DIALOG_TAXI: //Аренда такси 145
		{
		    if(response) {
		        ...
		        SCM(playerid, COLOR_LIGHTGREEN, "Для того, чтобы начать работу таксиста {FFDF0F}нажмите 2");
		    } else {
		        RemovePlayerFromVehicle(playerid);
		    }
		}
		...
		case DIALOG_BUS: //Аренда автобуса
		{
		    if(response) {
				if(player_info[playerid][MONEY] < 180) {
                    RemovePlayerFromVehicle(playerid);
					return SCM(playerid, COLOR_LIGHTGREY, "У Вас недостаточно денег");
				}
		        player_info[playerid][MONEY]-= 180;
				GameTextForPlayer(playerid, "~r~-180$", 3000, 1);
			    static const fmt_query[] = "UPDATE `accounts` SET `money` = '%d' WHERE `id` = '%d'";
				new query[sizeof(fmt_query)+(-2+9)+(-2+8)];
				format(query, sizeof(query), fmt_query, player_info[playerid][MONEY], player_info[playerid][ID]);
				mysql_query(dbHandle, query);
				SetPVarInt(playerid, "bus_work", 1);
                jobdriver[GetPlayerVehicleID(playerid)] = playerid;
		        SCM(playerid, COLOR_LIGHTGREEN, "Для того, чтобы начать работу водителя автобуса {FFDF0F}нажмите 2");
		    } else {
		        RemovePlayerFromVehicle(playerid);
		    }
		}
		case DIALOG_BUS + 1:
		{
            if(response) {
                new fare;
			    if(sscanf(inputtext, "d", fare)) return SPD(playerid, DIALOG_BUS + 1, DIALOG_STYLE_INPUT, "{ffcd00}Стоимость проезда", "{ffffff}Сколько будет стоить проезд?\nУкажите цену (0$ - 100$)", "Далее", "Отмена");
				if(fare < 1 || fare > 100) return SPD(playerid, DIALOG_BUS + 1, DIALOG_STYLE_INPUT, "{ffcd00}Стоимость проезда", "{ffffff}Сколько будет стоить проезд?\nУкажите цену (0$ - 100$)", "Далее", "Отмена");
				SetPVarInt(playerid, "bus_fare", fare);
				SPD(playerid, DIALOG_BUS + 2, DIALOG_STYLE_LIST, "{ffcd00}Выберите маршрут", "1. Городской Лос-Сантос №1\n2. Городской Лос-Сантос №2\n3.Городской Сан-Фиерро\n4. Городской Лас-Вентурас\n5. Лос-Сантос — Сан-Фиерро\n6. Лос-Сантос — Лас-Вентурас\n7. Сан-Фиерро — Лас-Вентурас\n8. ЖД Вокзал ЛС — Завод\n9. Пригородный ЛВ\n10. ЖД Вокзал ЛС — Шахта", "Ок", "Отмена");
			}
		}
		case DIALOG_BUS + 2:
		{
		    if(response) {
				switch(listitem) {
				    case 0:
				    {
				        ...
				        new string[81];
						format(string, sizeof(string), "%s начал работу водителя автобуса на маршруте Городской Лос-Сантос №1", player_info[playerid][NAME]);
						ProxDetector(30.0, playerid, string, 0xde92ffFF, 0xde92ffFF, 0xde92ffFF, 0xde92ffFF, 0xde92ffFF);
		...
		case DIALOG_BUS + 3:
		{
		    if(response) {
		        RemovePlayerFromVehicle(playerid);
                SetPVarInt(playerid, "bus_work", 0);
				SCM(playerid, COLOR_YELLOW, "Рабочий день завершён");

		        new stringOne[29], stringTwo[80], stringThree[33];
				format(stringOne, sizeof(stringOne), "Заработано: {5EFF36}%d$", GetPVarInt(playerid, "bus_salary"));
		        format(stringTwo, sizeof(stringTwo), "Количество пассажиров: {5EFF36}%d{ffffff}. Куплено билетов на {5EFF36}%d$.", GetPVarInt(playerid, "bus_passengers"), GetPVarInt(playerid, "bus_passengers") * GetPVarInt(playerid, "bus_fare"));
		        format(stringThree, sizeof(stringThree), "За ремонт автобуса: {FFDF0F}-%d$", 34);
				SCM(playerid, COLOR_WHITE, stringOne);
				SCM(playerid, COLOR_WHITE, stringTwo);
				SCM(playerid, COLOR_WHITE, stringThree);
				SCM(playerid, COLOR_LIGHTGREEN, "Деньги будут перечислены на Ваш счет во время зарплаты");
				...
				{
			   		SetVehicleToRespawn(busveh[playerid]);
				}
			   	busveh[playerid] = -1;
		    }
		}
...
forward nextcheckbus(playerid);
public nextcheckbus(playerid)
{
    nowcheck[playerid]++;
...
forward busworkend(playerid);
public busworkend(playerid)
{
	if(GetPVarInt(playerid, "bus_work") ==  1)
    {
        SetPVarInt(playerid, "bus_work", 0);
...
CMD:veh(playerid, params[])
{
    ...
    if(sscanf(params, "ddd", params[0],params[1],params[2])) return SCM(playerid, COLOR_LIGHTGREY, "Используй /veh [id машины] [цвет 1] [цвет 2]");
    {
		...
		format(string, sizeof(string), "[A] %s[%d] создал автомобиль %s [ID: %d]", player_info[playerid][NAME], playerid, VehicleNames[params[0]], params[0]);
    	SCMA(COLOR_GREY, string);
